"""
Team Experiment — Neural Network Classifier (predict SepsisLabel)
 
Decision change vs. baseline:
- CHANGED: Shallow MLP (2 hidden layers, 64→32 units) → deeper MLP (4 hidden layers,
  128→64→32→16 units).
  Rationale: The baseline architecture is intentionally minimal. A deeper network
  with more units can learn higher-order feature interactions (e.g. combinations of
  vitals that together signal sepsis onset) that a 2-layer network may miss.
- CHANGED: Epochs 10 → 20. Deeper networks converge more slowly; 10 epochs is
  insufficient for the additional layers to learn meaningful representations
  so it felt necessary to add.
 
Hypothesis: The deeper architecture will improve the model's ability to capture complex
relationships in the data, leading to better discrimination of sepsis cases. Leading to an
increase in recall and F1 scores.

Current results: Slight improvement in recall and F1 which makes sense as this design decision
doesn't fix the core issue with the baseline.
...
N_SPLITS    = 5
BATCH_SIZE  = 1024
EPOCHS      = 20       # increased from 10 — deeper model needs more time to converge (may be changed)
LR          = 1e-3
 
# Design Decision: Deeper architecture — 4 hidden layers instead of 2
HIDDEN1 = 128          # was 64
HIDDEN2 = 64           # was 32
HIDDEN3 = 32           # new
HIDDEN4 = 16           # new
...
print(f"\nArchitecture: input -> {HIDDEN1} -> {HIDDEN2} -> {HIDDEN3} -> {HIDDEN4} -> 1")
...
for label, X, y in [("val", X_val_final, y_val),
    ...
    pd.DataFrame(cm,
        ...
        os.path.join(RESULTS_DIR, f"deep_nn_confmat_{label}.csv")
    )
 
out_path = os.path.join(RESULTS_DIR, "deep_nn_metrics.csv")
